...
const GameScreen = ({ route, navigation }) => {
  ...
  const handleGameOver = useCallback(
    async (score, coins) => {
      ...
      if (score > prevBest) {
        ...
        audioServiceV4.playVoiceOver('best_score');
      }
      ...
      if (score >= level.scoreToAdvance && levelId < LEVELS.length) {
        ...
        audioServiceV4.playVoiceOver('level_unlocked');
      }
  ...
  useEffect(() => {
    ...
    audioServiceV4.initialize();
    
    // Démarrer la musique de fond
    if (gameState === "countdown") {
      audioServiceV4.playBackgroundMusic(levelId);
    }
  ...
  useEffect(() => {
    ...
    return () => {
      audioServiceV4.stopBackgroundMusic();
    };
  }, []);
